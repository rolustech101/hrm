...
function getCand($email, $phone)
{
    if (isset($email) && !empty($email)) {
        ...
        if ($res->num_rows > 1) {
            ...
            $cid = $row['cand_id'];
            $GLOBALS['candidate_guid'] = $cid;
            $c = BeanFactory::getBean('RT_Candidates', $cid);
        } else {
            $new_id = create_guid();
            $GLOBALS['is_new'] = true;
            $GLOBALS['candidate_guid'] = $new_id;
...
if($candidate == ''){
    ...
    if($GLOBALS['is_new']){
        $candidate->id = $GLOBALS['candidate_guid'];
        $candidate->new_with_id = true;
    ...
    $target_file = $target_dir .'/'. $GLOBALS['candidate_guid']; //basename($_FILES["fileToUpload"]["name"]);
    $uploadOk = 1;
    clearstatcache();
...
// if everything is ok, try to upload file
    ...
    $candidate->file_mime_type = $_FILES["fileToUpload"]["type"];
    $candidate->filename = $_FILES["fileToUpload"]["name"];
    $candidate->save();
