...
class ListView
{
    ...
    function processListView($seed, $xTemplateSection, $html_varName)
    {
        ...
        if (isset($seed->module_dir) && $populateOnly) {
            if (empty($GLOBALS['displayListView']) && strcmp(strtolower($_REQUEST['action']),
                    'popup') != 0 && (!empty($_REQUEST['clear_query']) || $_REQUEST['module'] == $seed->module_dir && ((empty($_REQUEST['query']) || $_REQUEST['query'] == 'MSI') && (empty($_SESSION['last_search_mod']) || $_SESSION['last_search_mod'] != $seed->module_dir)))
            ) {
        ...
        if (!isset($_REQUEST['action'])) {
            ...
            $this->shouldProcess = is_subclass_of($seed, "SugarBean")
                && (($_REQUEST['action'] == 'index') || ('ListView' == substr($_REQUEST['action'], 0,
                            8)) /* cn: to include all ListViewXXX.php type views */)
    ...
    {
        $GLOBALS['log']->fatal('in the custom LISTVIEW');
        ...
        while (list($aVal, $aItem) = each($data)) {
                ...
                if ($usage == 'query_only' && !empty($list_field['force_query_only_display'])) {
                    ...
                    if ($usage != 'query_only') {
                        ...
                        if (isset($aItem->field_defs[strtolower($list_field['name'])])) {
                            ...
                            if ($tmpField != null) {
                                $widget_contents = SugarFieldHandler::displaySmarty($list_field['fields'], $vardef,
                                    'ListView', $list_field);
                            } else {
                            ...
                            if (isset($list_field['widget_class']) && $list_field['widget_class'] == "SubPanelIcon") {
                                ...
                                if ((($list_field['name'] === 'edit_button' && $field_acl['EditView']) || ($list_field['name'] === 'cand_hired_button' && $field_acl['EditView']) || ($list_field['name'] === 'close_button' && $field_acl['EditView']) || ($list_field['name'] === 'remove_button' && $field_acl['Delete'])) && '' != ($_content = $layout_manager->widgetDisplay($list_field))) {
            ...
            if (!empty($button_contents)) {
                ...
                if (!empty($button_contents)) {
                    ...
                    $tempid = create_guid();
                    array_unshift($button_contents,
                        "<div style='display: inline' id='$tempid'>" . array_shift($button_contents) . "</div>");
    ...
    function getSessionVariable($localVarName, $varName)
    {
        ...
        if (isset($_REQUEST[$this->getSessionVariableName($localVarName, $varName)])) {
            $this->setSessionVariable($localVarName, $varName,
                $_REQUEST[$this->getSessionVariableName($localVarName, $varName)]);
        }
    ...
    function getUserVariable($localVarName, $varName)
    {
        ...
        if (isset($_REQUEST[$this->getSessionVariableName($localVarName, $varName)])) {
            ...
            $this->setUserVariable($localVarName, $varName,
                $_REQUEST[$this->getSessionVariableName($localVarName, $varName)]);
        }
    ...
    function processSugarBean($xtemplateSection, $html_varName, $seed)
    {
        ...
        $filter = array();
        $ret_array = $seed->create_new_list_query($this->query_orderby, $this->query_where, $filter, $params, 0, '',
            true, $seed, true);
        if (!is_array($params)) {
            $params = array();
        }
        ...
        $this->processListNavigation($xtemplateSection, $html_varName, $current_offset, $next_offset, $previous_offset,
            $row_count, null, null, empty($seed->column_fields) ? null : count($seed->column_fields));
    ...
    function processUnionBeans($sugarbean, $subpanel_def, $html_var = 'CELL')
    {
        ...
        if (!empty($this->response)) {
            ...
            $response = SugarBean::get_union_related_list($sugarbean, $this->sortby, $this->sort_order,
                $this->query_where, $current_offset, -1, $this->records_per_page, $this->query_limit, $subpanel_def);
        ...
        global $list_view_row_count;
        $list_view_row_count = $row_count;
        $this->processListNavigation('dyn_list_view', $html_var, $current_offset, $next_offset, $previous_offset,
            $row_count, $sugarbean, $subpanel_def);
     ...
     * Contributor(s): ______________________________________..
     */
    function processListNavigation(
        $xtemplateSection,
        $html_varName,
        $current_offset,
        $next_offset,
                ...
                || (!empty($sugar_config['admin_export_only'])
                    && !(
                        ...
                        || (ACLController::moduleSupportsACL($_REQUEST['module'])
                            && ACLAction::getUserAccessLevel($current_user->id, $_REQUEST['module'],
    ...
    function processListRows($data, $xtemplateSection, $html_varName)
    {
        ...
        while (list($aVal, $aItem) = each($data)) {
            ...
            if ($this->shouldProcess) {
                ...
                $this->xTemplate->assign('CHECKALL',
                    "<input type='checkbox' class='checkbox'  title='" . $GLOBALS['app_strings']['LBL_SELECT_ALL_TITLE'] . "'  name='massall' id='massall' value='' onclick='sListView.check_all(document.MassUpdate, \"mass[]\", this.checked)'>");
            }
            if (!isset($this->data_array)) {
